...
export class RiotSyncWorker {
	...
	async tick(): Promise<void> {
		if (this.running || !this.leases.acquire("riot-sync", 10 * 60_000)) return;
		this.running = true;
		try {
			for (const row of this.registrations.dueRiotSync(Date.now(), this.config.RIOT_SYNC_BATCH_SIZE)) {
				...
				if (result.kind === "success")
					this.registrations.updateCanonicalRiotIdentity(
						row,
					...
					this.registrations.updateSync(row.guildId, row.userId, {
			...
			this.running = false;
			this.leases.release("riot-sync");
		}
	}

	async syncOne(guildId: string, userId: string): Promise<boolean> {
		const row = this.registrations.get(guildId, userId);
		...
		this.registrations.updateCanonicalRiotIdentity(row, result.account.gameName, result.account.tagLine, Date.now() + this.config.RIOT_SYNC_INTERVAL_DAYS * 86_400_000);
		return true;
	}
}
