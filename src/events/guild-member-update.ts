...
export function createGuildMemberUpdateHandler(
	...
	return async (oldMember: GuildMember | PartialGuildMember, newMember: GuildMember): Promise<void> => {
		...
		if (!parsed || (await isCurrentProjection(registrations, nicknameService, newMember))) return;
		try {
			...
			if (!actor || actor.id === newMember.client.user.id || (!permissions.isStaff(actor) && !permissions.isAdministrator(actor))) {
				await registrations.requestReconciliation(newMember.guild.id, newMember.id);
				await audits.create({
			...
			await registrations.upsertJoined(newMember.guild.id, newMember.id, newMember.user.username, newMember.joinedTimestamp ?? Date.now());
			...
			if (result.kind !== "success") await registrations.requestReconciliation(newMember.guild.id, newMember.id);
...
async function isCurrentProjection(registrations: RegistrationRepository, nicknames: NicknameService, member: GuildMember): Promise<boolean> {
	const row = await registrations.get(member.guild.id, member.id);
	if (!row) return false;
